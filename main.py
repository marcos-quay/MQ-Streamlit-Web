...
def update_coaches(selected_sport, video_names, coaches):

    for idx, coach in enumerate(coaches):
        doc_ref = db.collection(collection_name).document(coach)
        doc_ref.update({
            'videos_assigned': firestore.ArrayUnion([f'https://storage.googleapis.com/{bucket_name}/MQ-Videos-All/{selected_sport}/{video_name}.mp4' for video_name in video_names])
        })
...
def reset_coaches():
    ...
    for doc in docs:
        doc.reference.update({"videos_assigned": []})
...
def create_pie_chart():
    ...
    for doc in docs:
        ...
        if doc_dict['videos_assigned']:
            for video in doc_dict['videos_assigned']:
                video_name = video.split('/')[-1].split('.')[0]
                if pie_chart_data.get(video_name) is None:
                    pie_chart_data[video_name] = 0
                pie_chart_data[video_name] += 1
...
def main():
    ...
    videos = get_category_video_names()

    with st.sidebar:
        selected = st.radio("Select Option", [
                            "Assign Videos", "Add New Coaches", "Delete Coaches", "Add New Videos"])
    ...
    if selected == "Assign Videos":
        ...
        if st.button("Update Coaches"):
            if video_name and edited_coaches_emails and selected_sport:
                update_coaches(selected_sport, video_name,
                               edited_coaches_emails)
                st.success(
